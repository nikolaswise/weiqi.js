...
function groupAtPosition(size, board, position) {
  ...
  var search = function search(_x2, _x3, _x4) {
    var _again = true;
    ...
    _function: while (_again) {
      ...
      if (color === board.get(pos)) {
        ...
        _x4 = visited.add(pos);
        _again = true;
        pos = newGroup = newQueue = undefined;
        continue _function;
      }

      if (!board.has(pos)) {
        var newGroup = new _records.Group({ stones: group.get('stones'),
          liberties: group.get('liberties').add(pos) });
        _x2 = newGroup;
        _x3 = queue.shift();
        _x4 = visited.add(pos);
        _again = true;
        pos = newGroup = newQueue = newGroup = undefined;
        continue _function;
      }
...
function placeStone(board, move) {
  var allowSuicide = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];
...
function areaScore(game) {
  var komi = arguments.length <= 1 || arguments[1] === undefined ? 0.0 : arguments[1];
...
var Position = new _immutable2['default'].Record({ i: 0, j: 0 });
